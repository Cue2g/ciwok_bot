...
export async function addGroup(ctx:ContextFix){
    try {
        const messageText = ctx.update.message?.text
        const chat = ctx.chat;
        if(!chat) return ctx.reply('No se pudo obtener el id del chat, intentelo de nuevo');
        ...
        if(!messageText) throw new Error('MessageText Error');
